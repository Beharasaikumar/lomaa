...
//-------data science BACKEND CODE-------------------------------------------------

// Registration for react
app.post("/ds-register", async (req, res) => {
    console.log(req.body);
    const { user_name, email, password } = req.body;
    if (!user_name || !password || !email) {
        return res.status(400).send('Please fill in all fields.');
    ...
    try {
        const hashedPassword = await bcrypt.hash(password, 10);
        connection.query(
            'INSERT INTO ds_users (user_name, email, password) VALUES (?, ?, ?)',
            [user_name, email, hashedPassword],
            (err, results) => {
                if (err) {
                         if (err.code === 'ER_DUP_ENTRY') {
                            return res.status(400).send('<script>alert("Email already registered."); window.location.href = "./ds_frontend/index.html";</script>');                           } 
                           
                            return res.status(500).send('<script>alert("Database error."); window.location.href = "./ds_frontend/index.html";</script>');
                        }
                        res.status(200).send('<script>alert("Registration successful! Redirecting to login page..."); window.location.href = "./ds_frontend/index.html";</script>');
                    }
                );
            } catch (error) {
                res.status(500).send('<script>alert("Server error."); window.location.href = "./ds_frontend/index.html";</script>');
            }
   });
// Login  for react
app.post("/ds-login", async (req, res) => {
    const { email, password } = req.body;
    ...
    try {
        connection.query("SELECT * FROM ds_users WHERE email = ?", [email], async (err, results) => {
            if (err) {
                console.error(err.message);
                return res.status(500).send('<script>alert("DataBase error.");</script>');
            }
            ...
            if (results.length === 0) {
                return res.status(400).send('<script>alert("user not found"); window.location.href = "./ds_frontend/index.html";</script>');
            }
            ...
            const isMatch = await bcrypt.compare(password, user.password);
            if (!isMatch) {
                return res.status(401).send('<script>alert("invalid credentials"); window.location.href = "./ds_frontend/index.html";</script>');
            }
            
            res.send('<script>alert("Login successful!");window.location.href = "./ds_frontend/html/index.html";</script>');
        });
...
let verification = {}; // Store temporary verification codes 

// 📧 1️⃣ Send Verification Code via Email
app.post("/ds-send-code", async (req, res) => {
    const { email } = req.body;
...
// ✅ 2️⃣ Verify Code
app.post("/ds-verify-code", (req, res) => {
    const { email, code } = req.body;
...
// 🔐 3️⃣ Reset Password
app.post("/ds-reset-password", async (req, res) => {
    const { email, newPassword } = req.body;
    ...
    connection.query(
        "UPDATE ds_users SET password = ? WHERE email = ?",
        [hashedPassword, email],
        (err, results) => {
            if (err) {
                console.error("Database error:", err);
            ...
            delete verification[email]; // Remove used code
            res.json({ success: true, message: "Password reset successful!" });
        }
    );
});
